...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Du befindest dich auf dem falschen Teamspeak. Bitte betrete den Server: <font color="#f06d01">${voip.plugin_data.TSServer}</font>`;
			screenMessage = 'Falscher TeamSpeak Server';
			color = 'orangered';
			break;
		case WRONG_CHANNEL:
			msg = `Du befindest dich im Falschen Channel. Bitte betrete den <font color="#f06d01">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
			screenMessage = 'Falscher Channel';
			color = 'orangered';
			break;
		case INCORRECT_VERSION:
			msg = 'Using incorrect plugin version';
			screenMessage = 'Falsche Plugin Version. Bitte im Support melden!';
			color = 'green';
			break;
		case OK:
			color = '#04B431';
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[Generation-V] ${msg}</font>`;
	}
...
function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `Joine auf diesem TeamSpeak Server: <font color="#f06d01">${voip.plugin_data.TSServer}</font>`;
	document.getElementById('TSChannel').innerHTML = `Joine auf diesen TeamSpeak Channel: <font color="#f06d01">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
